...
unsigned int findToken(st_darts* handler, const char* str, unsigned int uLen)
{
	...
	st_darts_state* dState=&state;

	for (int i = 0; i < uLen; ++i){
		uint16_t iCode = stUTF8Decode((BYTE**)&str);
		if (iCode > 0 ){
			stLog("iCode=%u", iCode);
			ST_DARTS_STATE_SET_KEY(((st_darts_state*)&state), iCode);
...
void testModeCutWord(st_darts* pDarts, char* encode)
{
	...
	while (fgets(input, MAX_PATH, stdin)){
		...
		stDartsStateInit(pDarts, &dState, output, output + uLen);
		long long queryBeginTime = stTimer(stTimerType);
		stDartsCutWord(pDarts, &dState, output, wordId, wordPos, &uLen, 10);
		long long queryEndTime = stTimer(stTimerType);
...
void testModeCutWordForByte(st_darts* pDarts, char* encode)
{
	...
	char input[MAX_PATH];
	char output[MAX_PATH];
	struct st_wordInfo wordInfo[1024] = { {0, 0, 0} };
	char outWord[MAX_PATH];
	//memset(input, 0, sizeof(input));
	//memset(output, 0, sizeof(output));
	st_darts_state dState;
	...
	while (fgets(input, MAX_PATH, stdin)){
		...
		long long queryBeginTime = stTimer(stTimerType);
		uLen = 1024;
		stDartsCutWordByte(pDarts, &dState, output, output+uLen, wordInfo, &uLen, 10);
		long long queryEndTime = stTimer(stTimerType);
		stLog("result=%u, cost time=%lldus", uLen, queryEndTime - queryBeginTime);
		...
		for (int i = uLen - 1; i >= 0; --i){
			...
			stLog("query i=%d, word=%s, wordId=%d, len=%u",
					(uLen - 1 - i), outWord, wordInfo[i].wordId, wordInfo[i].wordLen);
		}
		printf("Plz input # ");
		fflush(stdout);
	}
...
void testModeCutWordComplete(st_darts* pDarts, char* encode)
{
	st_timer stTimerType = ST_TIMER_MICRO_SEC;
	char input[MAX_PATH];
	char output[MAX_PATH];
	...
	while (fgets(input, MAX_PATH, stdin)){
		...
		long long queryBeginTime = stTimer(stTimerType);
		uLen = 0;
		while (uLen < 1024 && stDartsNextWord(pDarts, &dState, &wordInfo[uLen])){
			++uLen;
		}
		long long queryEndTime = stTimer(stTimerType);
		stLog("result=%u, cost time=%lldus", uLen, queryEndTime - queryBeginTime);
...
void usage()
{
	printf("usage: ./st_tools <dictMode> <task> [option] -f dictfile\n"
			"  dictMode:\n"
			"	-s: load word from word list file\n"
			"	-l: load dictionary from darts file\n"
...
int main(int argc, char *argv[])
{
	...
	int c = 0;
	while ((c = getopt(argc, argv, "slpf:c:t:n:u:m:h")) != -1){
		switch(c){
			case 'h':
				usage();
				return 0;
			case 's':
				bModeWordList = 0;
				stLog("**  dictionary mode -%c", (char)c);
				break;
			case 'l':
				bModeWordList = 1;
				stLog("**  dictionary mode -%c", (char)c);
			...
			case 'f':
				...
				stLog("**  dictionary file %s", optarg);
				break;
			case 'c':
				if (strlen(optarg) >= MAX_PATH){
					stErr("dictionary mode -%c=%s, path too long, >%d", (char)c, optarg, MAX_PATH);
	...
	else if (bModeWordList == 2){
		g_s_pDarts = stDartsLoadMmap(dictFile);
	}
	else {
